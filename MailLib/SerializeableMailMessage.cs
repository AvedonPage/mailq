...
/* 
    ...
    Updated to match updated interface of MailMessage:
        - ReplyTo -> ReplyToList

    Michael Hansen (mihansen@microsoft.com)
    ...
    public class SerializableMailMessage : IXmlSerializable
    {
        ...
        public void ReadXml(XmlReader reader)
        {
            ...
            if (replyToNode != null)
            {
                foreach (XmlNode node in replyToNode.ChildNodes)
                {
                    ...
                    if (node.Attributes["DisplayName"] != null)
                        replyToDisplayName = node.Attributes["DisplayName"].Value;
                    MailAddress replyToAddress = new MailAddress(node.InnerText, replyToDisplayName);
                    this.Email.ReplyToList.Add(replyToAddress);
                }
            }
        ...
        public void WriteXml(XmlWriter writer)
        {
            if (this.Email != null)
            {
                ...
                if (this.Email.ReplyToList.Count > 0)
                {
                    writer.WriteStartElement("ReplyTo");
                    writer.WriteStartElement("Addresses");
                    foreach (MailAddress address in this.Email.ReplyToList)
                    {
                        writer.WriteStartElement("Address");
                        if (!string.IsNullOrEmpty(address.DisplayName))
                            writer.WriteAttributeString("DisplayName", address.DisplayName);
